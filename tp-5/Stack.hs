...
where
data Stack a = St [a] Int deriving Show
--                xs   n
{- INV. REP:
   ...
   VÁLIDO:
   * push 5 (St [1, 2] 2) = (St [5, 1, 2] 3)
   * pop (St [5, 1, 2] 3) = (St [1, 2] 2)
   * (St [5, 1, 2] 3)
   * (St [] 0)
   INVÁLIDO:
   * push 5 (St [1, 2] 2) = (St [1, 2, 5] 3) o (St [1, 5, 2] 3)
   * pop (St [5, 1, 2] 2) = (St [5, 1] 2) o (St [5, 2] 2)
   * isEmptyS (St [5, 1, 2] 3)
   * (St [5, 1, 2] 0)
   * (St [] 5)
    -}
...
top (St [] _) = error "la pila no tiene elementos"
top (St (x:xs) _) = x 
...
-- Precondición: la pila debe contener elementos
pop (St [] _) = error "la pila no tiene elementos"
